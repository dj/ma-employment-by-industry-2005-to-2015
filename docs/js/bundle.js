...
var _smallMultiplesVis = __webpack_require__(463);
...
function transformData(rows) {
	var data = rows.map(function (row) {
		var label = row.Industry;
		var industry = row.Industry.toLowerCase().split(' ')[0];
		var min = row.Min;
		var max = row.Max;
...
var renderSmallMultiples = (0, _smallMultiplesVis.SmallMultiplesVis)();

d3.csv('data/ma-employment-by-industry.csv').row(function (r) {
	return r;
}).get(function (data) {
	d3.select('#main').datum(transformData(data)).call(renderSmallMultiples);
});
...
exports.SmallMultiplesVis = SmallMultiplesVis;

var _d = __webpack_require__(90);

var d3 = _interopRequireWildcard(_d);

var _colors = __webpack_require__(464);

var c = _interopRequireWildcard(_colors);

var _months = __webpack_require__(465);
...
function SmallMultiplesVis() {
	...
	function render(selection) {
		...
		var xScale = d3.scaleTime().domain([start, end]).range([0, width]);
		...
		selection.each(function (data) {
			...
			data.forEach(function (row) {
				yScale[row.industry] = d3.scaleLinear().domain([row.max, row.min * 0.9]).range([0, height]);
				area[row.industry] = d3.area().x(function (d) {
					return xScale(new Date(d[0]));
				}).y0(height).y1(function (d) {
					return yScale[row.industry](d[1]);
				});
				line[row.industry] = d3.area().x(function (d) {
					return xScale(new Date(d[0]));
				}).y(function (d) {
					return yScale[row.industry](d[1]);
				});
			...
			var container = d3.select(this).selectAll('.chart').data(data);

			// Create an svg for each datum
			var chart = container.enter().append('svg').attr('width', rect.width / 2).attr('class', function (d) {
				return 'chart ' + d.industry;
			});

			var group = chart.append('g').attr('width', width).attr('transform', 'translate(0,0)').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('class', function (d) {
				return d.industry;
			});

			// x axis labels
			group.append('text').text('2005').attr('y', height + 20);

			group.append('text').attr('y', height + 20).attr('x', width - 35).text('2015');

			// industry label
			group.append('text').text(function (d) {
				return d.label;
			}).attr('y', -20).style('color', c.themePrimary3);

			// area
			group.append('path').style('stroke', c.themePrimary2).style('fill', c.themePrimary2).style('opacity', 0.3).attr('d', function (d) {
				return area[d.industry](d.values);
			}).style('stroke-width', 1.15);

			// plot lines
			group.append('path').style('stroke', c.themePrimary2).attr('d', function (d) {
				return line[d.industry](d.values);
			}).style('stroke-width', 1);

			// cursor: hightlight selected datum
			var cursor = group.append('line').attr('stroke', c.themePrimary3).attr('class', 'pointer').attr('opacity', 0).attr('stroke-width', 1.5).style('fill', c.themePrimary3);
			...
			group.on('mousemove', mousemove).on('mouseover', mouseover).on('mouseout', mouseout);
		});
	}
...
var themePrimary1 = exports.themePrimary1 = '#14558f';
var themePrimary2 = exports.themePrimary2 = '#43956f';
var themePrimary3 = exports.themePrimary3 = '#535353';
var themePrimary4 = exports.themePrimary4 = '#141414';
var themeSecondary1 = exports.themeSecondary1 = '#f6c51b';
var themeLightGrey = exports.themeLightGrey = '#f2f2f2';
var themeDarkGrey = exports.themeDarkGrey = '#dcdcdc';
...
var update = __webpack_require__(469)(content, options);
